...
#
def response_handler_getCamCtrl(response_data):
    #print(f"Server responded with: {response_data}")

    expo = int(response_data.get('getCCtrl_expo', 0))
    gain = int(response_data.get('getCCtrl_gain', 0))

    return expo, gain;

def response_handler_setCamCtrl(response_data):
    #print(f"Server responded with: {response_data}")

    retVal = response_data.get('set_camCtrl_status', 0)
...
def main():
    ...
    expo, gain = send_data_to_server(host, port, payload, receive_handler=response_handler_getCamCtrl, receive_timeout=5.0)
    
    print(f"expo {expo} - gain {gain}")
    ...
    retVal = send_data_to_server(host, port, payload, receive_handler=response_handler_setCamCtrl, receive_timeout=5.0)    
    
    print(f"set cctrl status: {retVal}")
